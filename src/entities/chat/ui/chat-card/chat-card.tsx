...
export const ChatCard: FC<ChatCardProps> = ({ item, isSelected }) => {
  ...
  return (
    <Card className={cn('w-full border-none shadow-none truncate hover:bg-muted rounded-none p-2 transition-colors', isCardSelected && 'dark:bg-slate-900 bg-gray-200')}>
      <Stack className="space-x-2">
        <Avatar>
          <AvatarFallback>{item.name.charAt(0).toUpperCase()}</AvatarFallback>
        </Avatar>
        <Stack direction="vertical" spacing={1} className="truncate">
